window.addEventListener("load", () => {
  ...
  input[type=checkbox].input-switch:checked,input[type=radio].input-switch:checked {
    ...
    // Alte Funktion: Durch eine von ChatGPT ersätzt

    // for dots instead of lines: <line x1="32" y1="11" x2="32" y2="11" stroke-linecap="round" stroke-width="10" stroke="${fg}"/></g></defs>
    // else (original): <line x1="32" y1="28" x2="32" y2="7" stroke-linecap="round" stroke-width="6" stroke="${fg}"/></g></defs>
    // third version: <line x1="32" y1="22" x2="32" y2="8" stroke-linecap="round" stroke-width="6" stroke="${fg}"/></g></defs>
    // Punkte -- Style -- Linie -- Design
    ...
    //   <line x1="32" y1="20" x2="32" y2="8" stroke-linecap="round" stroke-width="6" stroke="${fg}"/></g></defs>
    ...
    let makeKnobFrames = (fr, fg, bg, nostop) => {
        // Standart: 101 Frames
        if (nostop) fr = 301; //(FIX)

        let height = fr * 64;

        // hier irgendwie mehr abstufungen in den Frames einbauen (FIXME)

        let svg = `<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="64" height="${height}" viewBox="0 0 64 ${height}" preserveAspectRatio="none">
      <defs>
        <g id="K">
          <circle cx="32" cy="32" r="30" fill="${bg}"/>
          <line x1="32" y1="20" x2="32" y2="8" stroke-linecap="round" stroke-width="6" stroke="${fg}"/>
        </g>
      </defs>`;
        ...
        for (let i = 0; i < fr; ++i) {
            ...
            svg += `<use xlink:href="#K" transform="translate(0,${64 * i}) rotate(${angle},32,32)"/>`;
        }
  ...
  <circle fill="${fg}" cx="${w * 0.5}" cy="${h * 1.5}" r="${minwh * 0.25}"/></g></svg>`;
    ...
    let initKnobs = (el) => {
        ...
        el.refresh = () => {
            ...
            bg = el.getAttribute("data-bgcolor") || op.bgcolor;
            fg = el.getAttribute("data-fgcolor") || op.fgcolor;
            nostop = el.getAttribute("data-nostop") || op.nostop;
            // sensex und sensey für nostop höher um Messpuffer auszugleichen
            ik.sensex = ik.sensey = !nostop ? 200 : 600;
            ...
            if (src) {
                ...
                switch (ik.itype) {
                    case "k":
                        svg = makeKnobFrames(101, fg, bg, nostop);
                        break;
                    case "h":
        ...
        el.setValue = (v, fromInternal) => {
            ...
            if (nostop && !fromInternal) {
                let len = el.max - el.min;
                let maxAlt = +el.max - len / 3;
                let minAlt = +el.min + len / 3;

                if (v > maxAlt) {
                    el.setValue(v - 359, true);
                    if (ik.dragfrom) ik.dragfrom.v -= 359;
                    return;
                } else if (v < minAlt) {
                    el.setValue(v + 359, true);
                    if (ik.dragfrom) ik.dragfrom.v += 359;
        ...
        ik.pointerdown = (ev) => {
            ...
            // if (el.value < minAlt || el.value > maxAlt) {
            //     //     console.log("Wert außerhalb des Bereichs");
            // }
        ...
        ik.wheel = (ev) => {
            // console.log("soll hier der Fokus auf das Ojekt gelegt werden?");
        ...
        // wrap around wenn nostop == true
        // funktioniert noch nicht und soll auch dynamischer funktionieren
        if (nostop) {
            let minAlt = parseInt(el.getAttribute("min"));
            let maxAlt = parseInt(el.getAttribute("max"));
            let messpuffer = maxAlt - minAlt;

            el.min = parseInt(el.getAttribute("min")) - messpuffer;
            el.max = parseInt(el.getAttribute("max")) + messpuffer;

            el.refresh();
            svg = makeKnobFrames(101, fg, bg, nostop);
            el.style.backgroundImage = "url(data:image/svg+xml;base64," + btoa(svg) + ")";

            // Immer Werte zwischen 0 und 359 anzeigen und verwenden
            el.addEventListener("input", (e) => {
                let val = parseInt(el.value);
                let wrapped = ((val % 360) + 360) % 360;
                // Nur setzen, wenn nötig, um Endlosschleifen zu vermeiden
                if (el.value != wrapped) {
                    el.value = wrapped;
                    el.setAttribute("value", wrapped);
                    el.redraw();
                }
            });
        }

        // evtl Highlight zur Fokussierung
        el.addEventListener("focus", (e) => {
            // console.log("Input ist fokussiert");
            // svg = makeKnobFrames(101, "hsl(235, 35%, 53%)", bg, nostop);
            return;

            svg = makeKnobFrames(101, fg, "hsl(235, 35%, 53%)", nostop);
            el.style.backgroundImage = "url(data:image/svg+xml;base64," + btoa(svg) + ")";
        });
        el.addEventListener("blur", (e) => {
            // console.log("Input hat den Fokus verloren");
            svg = makeKnobFrames(101, fg, bg, nostop);
            el.style.backgroundImage = "url(data:image/svg+xml;base64," + btoa(svg) + ")";
        });
        ...
        el.addEventListener("dblclick", (e) => {
            // hier zurücksetzen
            let resetValue = el.getAttribute("data-reset") || op.resetValue;
            // console.log("Zurückgesetzt auf" + resetValue);
            el.setValue(resetValue);
        });
    };
    let refreshque = () => {
